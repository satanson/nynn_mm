		...
		for (int i=0;i<socket_num;i++){
			if (items[i].revents&ZMQ_POLLIN){
				...
				case prot::CMD_NOTIFY:{
					...
					break;}
				case prot::CMD_GET_SGSDIR:{
					handle_get_sgsdir(rep,*graph.get());
					break;}
				case prot::CMD_VTX_EXISTS:{
					handle_vtx_exists(rep,*graph.get());
					break;}
				case prot::CMD_GET_REMOTE:{
					uint16_t dport=rand_range(data_port_range_min,data_port_range_max);					
					handle_get_remote(rep,*graph.get(),glock,dport);
					break;}
				default:{
